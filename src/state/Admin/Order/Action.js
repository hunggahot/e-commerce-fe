...
export const shipOrder = (orderId) => {
  return async (dispatch) => {
    try {
      ...
      const { data } = await api.put(`/api/v1/admin/orders/${orderId}/ship`);
...
export const deliveredOrder = (orderId) => async (dispatch) => {
  ...
  try {
    const response = await api.put(`/api/v1/admin/orders/${orderId}/deliver`);
    const data = response.data;
...
export const cancelOrder = (orderId) => async (dispatch) => {
  ...
  try {
    const response = await api.put(`/api/v1/admin/orders/${orderId}/cancel`);
    const data = response.data;
